	...
	public void onCustomEvent(Event customEvent) {
		if (customEvent instanceof RDBMScoreQueryResultEvent) {
			if (((RDBMScoreQueryResultEvent) customEvent).getOwnerPlugin()
				...
				System.out
						.println("RDBMSlogger caught RDBMScoreQueryResultEvent, but ignored it, it's not the owner.");
			}
				...
				if (event.getEffectedRowCount() > -1) {
					...
					try {
						...
						while (crs.next()) {
							...
							// crs.getTimestamp("timeStamp") + ") " +
							// crs.getString("output"));
							// System.out.println(crs.getString("eventName"));

							// plugin.getServer().getPluginManager()
							// .registerEvent(crs.getString("eventName"),
							// plugin.eventListener, Event.Priority.Monitor,
							// plugin);

							// pm.registerEvent(crs.getString("eventName"),
							// plugin.eventListener, Event.Priority.Monitor,
							// plugin);
						}
